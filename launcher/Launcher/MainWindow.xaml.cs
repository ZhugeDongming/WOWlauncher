...
namespace WowSuite.Launcher
{
    public partial class MainWindow : Window
    {
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            //LocalConfiguration.Instance.Folders.CheckFoldersAsync();
            LoadConfig();
        ...
        private void LoadConfig()
        {
            ...
            if (registryKey != null)
            {
                string wow_dictory = (string)registryKey.GetValue("WOWDictory");
                registryKey.Close();
        ...
        private void Launcher_NewsLoadBanner(object sender, LoadTextEventArgs e)
        {
            switch (e.State)
            {
                ...
                case LoadingState.Completed:
                    ...
                    {
                        MessageBox.Show("可能无法获取新闻。请重启程序，如果问题仍然存在，请联系技术支持。");
        ...
        {
            //string wow_full_path = LocalConfiguration.Instance.wowGameDictory + "\\" + Wow.FileName.WOW_GAME_EXE_NAME;
            string wow_full_path = Path.Combine(LocalConfiguration.Instance.wowGameDictory, Wow.FileName.WOW_GAME_EXE_NAME);
            if (File.Exists(wow_full_path))
            {
                playButton.Content = "STARTING";
                playButton.IsEnabled = false;

                var process = new Process();
                process.StartInfo.FileName = wow_full_path;
                ...
                MessageBox.Show(this,
                    string.Format("文件 {0} 没有找到. 请在程序右上角的配置窗口选择魔兽世界游戏目录",
            ...
            //else
            //    MessageBox.Show(this,
            //        string.Format("文件 {0} 没有找到. 请将登录器放在魔兽世界游戏目录里面，例如：World of Warcraft",
            //            Wow.FileName.WOW_GAME_EXE_NAME));
        }

        private void CacheDeleteButton_Click(object sender, RoutedEventArgs e)
        {
            string cache_dic = LocalConfiguration.Instance.wowGameDictory;
            string cache = Path.Combine(cache_dic, LocalConfiguration.Instance.Folders.GetPath(WowLauncherFolder.Cache));
            //string cache = LocalConfiguration.Instance.Folders.GetPath(WowLauncherFolder.Cache);

            if (Directory.Exists(cache))
            {
                Directory.Delete(cache, true);
                MessageBox.Show(this, string.Format("文件 {0} 成功删除", AppFolder.CACHE_FOLDER_NAME), "Success");
            }
            else
            {
                MessageBox.Show(this, string.Format("文件 {0} 没有找到", AppFolder.CACHE_FOLDER_NAME), "Error");
            }
        }

        private void RealmListButton_Click(object sender, RoutedEventArgs e)
        {
            string localeFolder = Path.Combine(LocalConfiguration.Instance.wowGameDictory,Wow.FolderName.Client.DATA_FOLDER_NAME, Settings.Default.lang_wow);
            ...
            if (Directory.Exists(localeFolder))
            {
                using (var writer = new StreamWriter(realmListFile))
                {
                    ...
                    MessageBox.Show(this, "已添加服务器IP地址到游戏的realmlist", "Realmlist");
                }
            }
            else
            {
